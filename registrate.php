<form name="form1" method="post" action="">
  ...
  <tr>
	...
	if($email_separar2[$email_cual2] == "ru") {
		...
		@mysqli_query($conn, "INSERT INTO banip (ip, tiempo, motivo) VALUES ('$ip2', '$tiempo2', '$motivo2')");
	} else {
		if($user2==NULL || strlen($pass2) <= 5 || $email2 == NULL) {
			...
			$query_user2 = mysqli_query($conn, "SELECT * FROM users WHERE nombre='".$user2."'");
			if(mysqli_num_rows($query_user2)) {
				echo '<div class="error">Ese nombre de usuario ya esta ocupado.</div>';
			} else {
				$query_email2 = mysqli_query($conn, "SELECT * FROM users WHERE email='".$email2."'");
				if(mysqli_num_rows($query_email2)) {
					...
					if($pass2 != $pass22) {
						...
						if($resp->isSuccess()) {
							...
							$pass32 = md5(md5($pass));
							@mysqli_query($conn, "INSERT INTO users (nombre, pass, fnac, baneado, rango, fichas, email, activado, codeact, placas, puesta, visible) VALUES ('$user2', '$pass32', '$fnac2', '0', 'USR', '100', '$email2', '0', '$codeact2', '', '', '1')");
							...
							@mysqli_query($conn, "UPDATE users SET ip='".$_SERVER['REMOTE_ADDR']."' WHERE nombre='$user2'");
